...
/**
 * ============================================================================
 * MIO Background Shader (WebGL2 + Bayer Dithering)
...
const float FLOW_SPEED = 0.55000000;
...
export type MioBackgroundShaderProps = {
  ...
  background?: { dark?: string; light?: string };
  /**
   * Opacidad general del canvas (0.0 = transparente, 1.0 = completamente opaco).
   * Para un fondo sutil que no opaque el texto, se recomienda entre 0.3 y 0.55.
   * Por defecto: 0.45
   */
  opacity?: number;
  time?: number;
...
function animate(
  ...
  function fitCanvas() {
    // Para un fondo ambiental difuso y dither, 1.0x de escala nativa es ultra-nítido
    // y ahorra hasta 75% de cómputo en pantallas Retina (2x/3x)
  ...
  let lastRenderNow = 0;
  const MIN_FRAME_INTERVAL_MS = 28; // ~35 FPS de actualización: fluido para fondo y ligero para la GPU
  ...
  function tick(now: number) {
    ...
    // Limitador de tasa de refresco para no ahogar la GPU ni competir con la interfaz interactiva
