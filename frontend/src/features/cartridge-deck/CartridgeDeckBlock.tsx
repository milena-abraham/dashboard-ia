...
// Sonidos hápticos sintetizados en tiempo real mediante Web Audio API
const playTactileSound = (
  type: 'mount' | 'eject' | 'click' | 'dial' | 'tab' | 'inference'
) => {
  ...
  try {
    ...
    if (type === 'mount') {
      // Encaje suave de cartucho deslizándose en la ranura
      ...
      // Eyección mecánica suave con resorte
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'sine';
      ...
      // Sonido de inicio de cómputo en lote
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'sine';
    ...
    // Silencioso si el audio está inactivo
  }
};
